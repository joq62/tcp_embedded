...
init([]) ->
    ...
    {files_dirs_to_keep,Keep}=lists:keyfind(files_dirs_to_keep,1,Info),
...
handle_cast({register_service,ServiceId,ServicePod}, State) ->
    io:format("Dbg ~p~n",[{?MODULE,?LINE,register_service,ServiceId,ServicePod}]),
    [rpc:cast(SdPod,sd_service,register_service,[ServiceId,ServicePod])||SdPod<-State#state.sd_pods], 
    {noreply, State};
