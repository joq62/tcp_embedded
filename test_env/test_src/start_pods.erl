...
%% --------------------------------------------------------------------
start()->
    StartR=start_pods(?PODS,[]),
    Result=case lists:keymember(error,1,StartR) of
	       false->
		   [{net_adm:ping(Pod),Pod,Service}||{ok,Pod,Service}<-StartR];
	       true->
		   StartR
	   end,
    {Result,StartR}.
...
start_pods([{NodeIdStr,PodId}|T],Acc)->
    ...
    % Controller pod to get controller:service
    PodController=list_to_atom(?POD_CONTROLLER_ID++"@"++Host),
    R=case rpc:call(PodController,controller_service,create_pod,[Node,PodId]) of
	  {ok,Pod}->
	      case rpc:call(PodController,controller_service,create_container,[Pod,PodId,"pod_controller_service"]) of
		  {ok,"pod_controller_service"}->
		      {ok,Pod,"pod_controller_service"};
		  {error,Err}->
		      [{error,Err}|Acc]
	      end;
	  {error,Err}->
    ...
    NewAcc=[R|Acc],
    io:format("~p~n",[{?MODULE,?LINE,NodeIdStr,PodId,R}]),
    timer:sleep(500),
    start_pods(T,NewAcc).
